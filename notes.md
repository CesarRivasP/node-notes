...
se va a aplicar globalmente, por lo que se debe ser administrador superusuario del equipo

- La bandera **-e** indica que debe estar pendiente de los archivos indicados por medio de su extensión
...
2.- No es una librería que es propia de node, es decir, es un paquete que se instala para su posterior utilización. No nativo de node, son expansiones, etc
...
- Otro de los objetos globales se llama **process**, no hay que definirlo en ningún lugar, puesto que node al ejecutarse crea dicha variable de entorno.
	* Este objeto global se encuentra corriendo a lo largo de toda la aplicación de node. También, este objeto es actualizado dependiendo del entorno en el que se encuentre corriendo
...
- El npm init proporciona una guia para la creación del package.json  
...
### Librerías
#### Nodemon
Funciona como un servidor de desarrollo, nos permite realizar cambios y que se actualicen en tiempo real
  * Mediante el comando **rs** se reinicia todo el proceso de nodemon, y se ejecuta de nuevo todo el programa.
#### Yargs
...
- **Parciales**: Es un lote de código html que se puede reutilizar
- **Helpers del hbs**: Es una función que se dispara cuando el template lo requiere
...
* **pick**: devuelve una copia del objeto, filtrando solo los valores que se quieren.

### Peticiones
#### Status de las peticiones
- **200**: se hizo correctamente
- **400**: Este error es producto de que no se envió la información como el servicio la espera
...
**Nota**: un **Bad Request** quiere decir que se solicito mal la información, se realizo la llamada al servicio mal
...
#### dirname
Indica que no importa el path en el que se encuentre, debe tomar ese path (es una variable global) y concatenarle la siguiente ruta
```
...
- Los **app.use** son middlewares, funciones que se van a disparar cada vez que la ejecución pase por su línea de ubicación
...
- **mLab**: Es un servicio gratuito de hasta 500 mb. Posteriormente se aliaron con mongodb en lo que deriva MongoDB Atlas
#### Modelo del usuario
...
* Son muy utilizadas en aplicación de hasta unos 5000 a 10000 usuarios mas o menos, y mediante estas se dificulta el acceso simultáneo de un usuarios desde diferentes equipos
...
- La estructura de **Json Web Token (JWT)** esta dividido en tres partes:
  + **Header**: usualmente se visualiza como la parte inicial y roja del token, y esta contiene información sobre el algoritmo utilizado para la encriptacion junto con el tipo de token. En este caso un JWT
    - **HS256**: ese algoritmo es una encriptación de doble vía, quiere decir, que aunque parezca que esta segura la información, la parte del payload es totalmente visible y si alguien obtiene ese token, podría ser capaz de poder obtener el payload, la fecha de emisión, expiración y otras cosas.
  ...
  La validación se realiza en el momento.
  En la parte de la firma es algo que se debe configurar en el backend para que de esa manera se puedan recibir los tokens y nos aseguremos que esos tokens no fueron manipulados y si lo fue se pueda detectar el cambio y que este no sea un token valido para la aplicación
...
#### Traer a valor presente tanto la fecha de expiración como la de emisión
```
let expira = new Date();
expira.setTime(<iat o exp> * 1000); //Para llevarlo al valor correcto
```
...
- Es común encontrar en los headers el token, muchas veces llamado **authorization**

### Local Storage
En el navegador, la pestaña de aplicación tiene una sección llamada **Local Storage**, en la cual se almacena información como las cookies, también se puede grabar información para que permanezca persistente en el equipo.
...
Se puede enviar el token a nuestro servidor mediante un POST https. El código para enviarla petición es mediante ajax.
